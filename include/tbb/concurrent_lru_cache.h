...
class concurrent_lru_cache : internal::no_assign {
  public:
    ...
    struct map_value_type {
        ...
        map_value_type(value_type const&                a_value,
                       ...
                       bool                             a_is_ready)
            : my_value(a_value), my_ref_counter(a_ref_counter),
              my_lru_list_iterator(a_lru_list_iterator), my_is_ready(a_is_ready) {}
    };
    ...
    typedef tbb::internal::aggregating_functor<self_type, aggregated_operation_type>
        aggregator_function_type;
    friend class tbb::internal::aggregating_functor<self_type, aggregated_operation_type>;
    typedef tbb::internal::aggregator<aggregator_function_type, aggregated_operation_type>
        aggregator_type;
  ...
  public:
    ...
    handle_object getOrCalculate(key_type k, const Func& user_func) {
        retrieve_aggregator_operation op(k);
        my_aggregator.execute(&op);
        if (op.is_new_value_needed()) {
            op.result().second.my_value = user_func();
            __TBB_store_with_release(op.result().second.my_is_ready, true);
        } else {
            tbb::internal::spin_wait_while_eq(op.result().second.my_is_ready, false);
        }
    ...
    class handle_object {
      ...
      public:
        ...
        handle_object(handle_object&& src)
            : my_cache_pointer(src.my_cache_pointer), my_map_record_ptr(src.my_map_record_ptr) {
            __TBB_ASSERT((src.my_cache_pointer && src.my_map_record_ptr) ||
        ...
        handle_object(handle_move_t m)
            : my_cache_pointer(&m.my_cache_ref), my_map_record_ptr(&m.my_map_record_ref) {}
...
#else
        friend handle_move_t move(handle_object& h) { return handle_object::move(h); }
        // TODO: add check for double moved objects by special dedicated field
  ...
  public:
    ...
    struct retrieve_aggregator_operation : aggregator_operation, private internal::no_assign {
        ...
        retrieve_aggregator_operation(key_type key)
            : aggregator_operation(aggregator_operation::op_retive), my_key(key),
    ...
    struct signal_end_of_usage_aggregator_operation : aggregator_operation,
  ...
  private:
    ...
    void signal_end_of_usage_serial(typename map_storage_type::reference map_record_ref) {
        ...
        __TBB_ASSERT(it != my_map_storage.end(),
